...
An append-only log of project checkpoints — one row per major iteration. It gives a human or a
fresh agent a quick history of how the project advanced and where it was left off.
...
(e.g. `1-foundations` → `1`), or blank when the branch starts with no number (see `CONTRIBUTING.md`).
